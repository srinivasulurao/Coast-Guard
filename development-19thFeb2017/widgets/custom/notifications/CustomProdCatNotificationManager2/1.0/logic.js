RightNow.namespace('Custom.Widgets.notifications.CustomProdCatNotificationManager2');
Custom.Widgets.notifications.CustomProdCatNotificationManager2 = RightNow.Widgets.extend({ 
    /**
     * Widget constructor.
     */
    ...
    */
    _displayMessage: function(message, type)
    {
        if(this._messageDiv) {
            ...
            var addBtn = this.Y.one(this.baseSelector + '_AddButton');
            RightNow.UI.displayBanner(message, {focus: true, type: type}).on('blur', function() { addBtn.focus(); });
        }
    }
});
