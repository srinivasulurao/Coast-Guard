RightNow.Widget.DynamicTextInput = function(data, instanceID) {
	this.data = data;
	...
	// setup hints when focus comes to field
	if (this.data.js.hint)
		YAHOO.util.Event.addListener(this._inputField, "focus", this._initializeHint(), null, this);
	// setup mask
	...
	// keep showing the hint while focused
	this._yuiOverlayManager = new YAHOO.widget.OverlayManager();
	this._hintFocused = false;
	this._inputFocused = false;
	// end of customization
};
RightNow.Widget.DynamicTextInput.prototype = {
	/**
	 * ---------------------------------------------- Form / UI Events and
	 * Functions: ----------------------------------------------
	 */
	...
	_onValidate : function(type, args) {
		...
		if (this._compareInputToMask(true) && this._checkRequired() && this._checkData() && this._checkValue() && this._checkEmail()) {
	...
	/**
	 * Validates that the input field has a value (if required) and that the
	 * value is of the correct format.
	 */
	...
	_checkValue : function() {
		if (this.data.js.type === RightNow.Interface.Constants.EUF_DT_INT) {
			...
			if (this._inputField.value !== ""
					&& (isNaN(Number(this._inputField.value)) || parseInt(this._inputField.value) !== parseFloat(this._inputField.value))) {
		...
		if (this.data.js.fieldSize) {
			...
			if (maxLength < length) {
				...
				errorString = RightNow.Text.sprintf(RightNow.Interface.getMessage("EXCEEDS_SZ_LIMIT_PCT_D_CHARS_PCT_D_LBL"), numtokExp, parseInt(maxLength
						/ roughMBCS));
	...
	/**
	 * Validation routine to check for valid strings in certain fields i.e.
	 * first_name, last_name and login
	 * 
	 * @param silent
	 *            Boolean Optional parameter: set to true if the caller wishes
	 *            to perform the validation check without displaying error
	 *            messages.
	 */
	_checkData : function(silent) {
		var angledBracketsRe = /<|>/;
		if (this._inputField.value !== "") {
			if (this._fieldName === "login") {
				var spacesRe = /\s/, doubleQuotesRe = /"/;
				...
				if (doubleQuotesRe.test(this._inputField.value)) {
					if (!silent)
						this._displayError(RightNow.Interface.getMessage('CONTAIN_QUOTE_CHARS_PLEASE_TRY_MSG'));
					return false;
				}
				// check if username contains angled brackets
				...
				// check if first_name or last_name contains angled brackets
				if (angledBracketsRe.test(this._inputField.value)) {
					if (!silent)
						this._displayError(RightNow.Interface.getMessage('NOT_CONT_EITHER_GT_LT_MSG'));
					return false;
	...
	_checkEmail : function(silent) {
		if (!(this._fieldName === 'email' || this._fieldName === 'email_alt1' || this._fieldName === 'email_alt2') || this._inputField.value === "")
			return true;
		if (!RightNow.Text.isValidEmailAddress(this._inputField.value)) {
			if (!silent)
				this._displayError(RightNow.Interface.getMessage("PCT_S_IS_INVALID_MSG"));
			return false;
		}
	...
	/**
	 * Trims the value of the input field (removes leading / trailing
	 * whitespace).
	 */
	_trimField : function() {
	...
	/**
	 * Shows hint on the input field's focus and hides the hint on the field's
	 * blur.
	 */
	_initializeHint : function() {
		YAHOO.util.Event.removeListener(this._inputField, this._initializeHint);
	...
	_displayError : function(errorMessage) {
		...
		if (commonErrorDiv) {
			...
			var errorLink = "<div><b><a href='javascript:void(0);' onclick='document.getElementById(\"" + this._inputField.id + "\").focus(); return false;'>"
	 ...
	 */
	/**
	 * Event handler for when email or login field blurs Check to see if the
	 * username/email is unique
	 */
	_checkExistingAccount : function() {
		if (this._inputField.value !== "" && this._inputField.value !== this.data.js.prev) {
			// static copy so we don't do a bunch of requests onblur if the
	...
	/**
	 * If args has a message and we aren't in the process of submitting then
	 * alert the message; otherwise no duplicate account exists
	 * 
	 * @param type
	 *            String Event name
	 * @param args
	...
	_initializeMask : function() {
		...
		if (this.data.attrs.always_show_mask) {
			...
			if (maskMessageOnPage && widgetContainer) {
				...
				YAHOO.util.Dom.addClass(messageNode, 'rn_Mask');
				this._maskNodeOnPage = widgetContainer.appendChild(messageNode);
			}
		}
	},
	/**
	 * Creates a mask array based on the passed-in string mask value.
	 * 
	 * @param mask
	 *            String The new mask to apply to the field
	 ...
	 * 
	 * @param submitting
	 *            Boolean Whether the form is submitting or not; don't display
	 *            the mask message if the form is submitting.
	 ...
	 * 
	 * @param error
	 *            Array Collection of details about error to display
	 */
	_showMaskMessage : function(error) {
		if (error === null) {
			...
			if (!this._showMaskMessage._maskMessages) {
				// set a static variable containing error messages so it's
				// lazily defined once across widget instances
			...
			for ( var i = 0, type; i < error.length; i++) {
				...
				if (type.charAt(0) === "F") {
					message += "<b>" + RightNow.Interface.getMessage('CHARACTER_LBL') + " " + (error[i][0] + 1) + "</b> "
							+ RightNow.Interface.getMessage('WAITING_FOR_CHARACTER_LBL') + type.charAt(1) + " ' <br/>";
					sampleMaskString[(error[i][0])] = "<span style='color:#F00;'>" + sampleMaskString[(error[i][0])] + "</span>";
				} else {
					if (type !== "MISS") {
						message += "<b>" + RightNow.Interface.getMessage('CHARACTER_LBL') + " " + (error[i][0] + 1) + "</b> "
								+ this._showMaskMessage._maskMessages[type] + "<br/>";
						if (type !== "LEN") {
							sampleMaskString[(error[i][0])] = "<span style='color:#F00;'>" + sampleMaskString[(error[i][0])] + "</span>";
						} else {
							break;
			...
			this._maskNode.body.innerHTML = RightNow.Interface.getMessage('EXPECTED_INPUT_LBL') + ": " + sampleMaskString + "<br/>" + message;
			this._maskNode.show();
		}
	},
	...
	_hideMaskMessage : function() {
		if (this._maskNode.cfg.getProperty("visible") !== false)
			this._maskNode.hide();
	}
};
