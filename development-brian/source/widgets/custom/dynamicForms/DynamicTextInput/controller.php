...
class DynamicTextInput extends DynamicFormInput
{
    ...
    function getData()
    {
        ...
        if(!($this->field instanceof CustomField))
        {
            if($this->field->data_type === EUF_DT_PASSWORD)
            {
                ...
                $this->data['value'] = '';
                $this->data['js']['prev'] = $this->field->value;
                $this->data['js']['passwordLength'] = getConfig(MYSEC_MIN_PASSWD_LEN);
                if($this->data['js']['passwordLength'] > 0 && !($this->fieldName === 'password'))
                    $this->data['attrs']['required'] = true;
            }
            elseif($this->field->data_type === EUF_DT_THREAD)
            {
                $this->data['value'] = '';
            }
